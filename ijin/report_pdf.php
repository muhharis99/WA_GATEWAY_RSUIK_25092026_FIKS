...
try {
    ...
    $sql = "
        SELECT phone, message, status, status_message, sent_at
    ...
    $mpdf->SetTitle('Laporan Pengiriman Ijin Dokter');
    ...
    <div class="header">
      <h1>LAPORAN PENGIRIMAN IJIN / PERUBAHAN PRAKTIK</h1>
      <p>' . e($hospitalName) . '</p>
    ...
    <table class="data"><thead><tr>
      <th width="5%">No</th><th width="17%">Tanggal</th><th width="14%">No. WhatsApp</th><th width="44%">Pesan</th><th width="9%">Status</th><th width="11%">Keterangan</th>
    </tr></thead><tbody>';

    if (!$rows) {
        $html .= '<tr><td colspan="6" class="center">Tidak ada data.</td></tr>';
    } else {
    ...
    $html .= '</tbody></table>';
    $mpdf->WriteHTML($html);
    $mpdf->Output('laporan-ijin-' . $startDate . '-sampai-' . $endDate . '.pdf', 'I');
    exit;
} catch (Throwable $e) {
    http_response_code(500);
    echo '<div style="font-family:Arial;padding:30px"><h2 style="color:#b91c1c">Laporan Ijin gagal dibuat</h2><pre>' .
